...
export function profileToInitialValues(p: { id?: number; [key: string]: unknown }): Record<string, string> {
  ...
  return {
    ...
    is_medical_frontliner_dependent: (p as { is_medical_frontliner_dependent?: boolean }).is_medical_frontliner_dependent
      ? "on"
      : "",
    study_destination_preference: String(
      (p as { study_destination_preference?: string }).study_destination_preference ?? "PHILIPPINES_ONLY"
    ),
